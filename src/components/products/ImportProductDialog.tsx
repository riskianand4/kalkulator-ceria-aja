...
export const ImportProductDialog: React.FC<ImportProductDialogProps> = ({
  ...
  const handleImport = async () => {
    const validProducts = parsedData.filter(p => p.validation.valid);
    ...
    try {
      // Remove validation dan rowIndex dari data sebelum kirim
      const productsToImport = validProducts.map(({ validation, rowIndex, ...product }) => product);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      onOpenChange(open);
        ...
        <DialogFooter className="flex-col-reverse sm:flex-row gap-2 sm:gap-0 pt-3 md:pt-4">
          {step === 'upload' && (
