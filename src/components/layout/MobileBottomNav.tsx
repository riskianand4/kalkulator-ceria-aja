...
export function MobileBottomNav() {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-around px-4 py-3 pb-safe mobile-safe-bottom">
        {filteredItems.map((item) => {
          ...
          return (
            <NavLink
              key={item.title}
              to={item.url}
              className="flex flex-col items-center justify-center min-w-0 flex-1 px-2"
            >
              <motion.div
                className={`flex flex-col items-center justify-center gap-1 p-2 transition-colors duration-200 ${
                  isActive 
                    ? 'text-primary' 
